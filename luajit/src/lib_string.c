  ...
  while (strfrmt < strfrmt_end) {
	...
	break;
      ...
      case 'q':
		...
		while ((v != 0) || (pos == 0) )
		{
			if (v < 1000)	sprintf(num[pos], "%i", (int)(v % 1000ULL) );
			else		sprintf(num[pos], "%03i", (int)(v % 1000ULL) );

			v = v / 1000;
			pos++;
			if (pos >= 127)
			{
				strcpy(num[pos], "overflow");
				break;
			}
		}
		tmp[0] = sign; 
		...
		for (i=0; i < l; i++)
		{
			if (form[i] == 'q') form[i] = 's';
		}
		sprintf(buff, form, tmp);
	}
	break;
      // value (e4 chunks + 1K)
      case 'V':
      case 'v':
        {
		long long value = num2intfrm(L, arg);

		char sign = (value < 0) ? '-' : 0;
		value = abs(value);
		int32_t frac = value % 10000;
		...
		if (strfrmt[-1] == 'v')
		{
			...
			{
				strcat(tmp, "_");
				sprintf(num[0], "%04i", frac); 
			}
			else
			{
				sprintf(num[0], "%4i", frac); 
			}
			strcat(tmp, num[0]); 
		}
 		size_t l = strlen(form);
