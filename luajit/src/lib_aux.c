...
LUALIB_API const char *luaL_split(lua_State *L, const char *s,
				 const char *p, const char *r)
{
  const char *wild;
  size_t l = strlen(p);
...
double get_clock_ns(void)
{
	struct timeval tv;
	gettimeofday(&tv, NULL);

	return (double)((uint64_t)tv.tv_sec *(uint64_t)1e9 +(uint64_t)tv.tv_usec * (uint64_t)1e3);
}

uint32_t aux_swap32(const uint32_t a)
{
	return (((a>>24)&0xFF)<<0) | (((a>>16)&0xFF)<<8) | (((a>>8)&0xFF)<<16) | (((a>>0)&0xFF)<<24);
}

uint16_t aux_swap16(const uint16_t a)
{
	return (((a>>8)&0xFF)<<0) | (((a>>0)&0xFF)<<8);
}

uint64_t aux_swap64(const uint64_t a)
{
	return (uint64_t)aux_swap32(a>>32ULL) | ( (uint64_t)aux_swap32(a) << 32ULL); 
}
