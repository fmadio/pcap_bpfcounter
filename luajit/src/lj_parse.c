...
typedef struct FuncScope {
  ...
  uint8_t iscontinueable;	/* Scope is a loop and allows a continue. */
} FuncScope;
...
/* Begin a scope. */
static void scope_begin(FuncState *fs, FuncScope *bl, int isbreakable, int iscontinueable)
{
  bl->breaklist = NO_JMP;
  bl->contlist = NO_JMP;
  bl->isbreakable = (uint8_t)isbreakable;
  bl->iscontinueable = (uint8_t)iscontinueable;
...
static void scope_end(FuncState *fs)
{
  ...
  if (bl->iscontinueable)
  {
    jmp_patch(fs, bl->contlist, fs->pc);
  }
}
...
static void parse_continue(LexState *ls)
{
  ...
  /* find continue level */
  int upval = 0;
  for (bl = fs->bl; bl && !bl->iscontinueable; bl = bl->prev)
  {
    upval |= bl->upval;  /* Collect upvalues in intervening scopes. */
  }
  //printf("cont %i %i\n", bl->isbreakable, bl->iscontinueable);

  if (!bl)  /* Error if no continuable scope found. */
    err_syntax(ls, LJ_ERR_XCONT);
...
static void parse_while(LexState *ls, BCLine line)
{
  ...
  scope_begin(fs, &bl1, 1, 0);		// scope for break block

  start = fs->lasttarget = fs->pc;
  condexit = expr_cond(ls);

  scope_begin(fs, &bl2, 0, 1);		// scope for cont block
  lex_check(ls, TK_do);
  loop = bcemit_AD(fs, BC_LOOP, fs->nactvar, 0);
  parse_block(ls);
  scope_end(fs);	// continue block

  jmp_patch(fs, bcemit_jmp(fs), start);
  lex_match(ls, TK_end, TK_while, line);

  scope_end(fs);	// break block
...
static void parse_repeat(LexState *ls, BCLine line)
{
  ...
  BCPos condexit;
  FuncScope bl1, bl2;
  scope_begin(fs, &bl1, 1, 1);  /* Breakable loop scope. */
  scope_begin(fs, &bl2, 0, 0);  /* Inner scope. */
...
static void parse_for_num(LexState *ls, GCstr *varname, BCLine line)
{
  ...
  // not sure why this is non breakable?
  scope_begin(fs, &bl, 0, 1);  /* Scope for visible variables. */
  var_add(ls, 1);
  bcreg_reserve(fs, 1);
  parse_block(ls);
  scope_end(fs);
...
static void parse_for(LexState *ls, BCLine line)
{
  ...
  GCstr *varname;
  FuncScope bl;
  scope_begin(fs, &bl, 1, 0);  /* Breakable loop scope. */
