//---------------------------------------------------------------------------------------------
//
// Copyright (c) 2018-2019, fmad engineering llc 
//
...
u64						s_Output_ESTimeout	= 10e9;
bool					s_Output_IsESNULL	= false;			// debug mode to run the output code just skip the final TCP push 

bool					s_Output_Keepalive	= false;			// ES connection would be keepalive/persistent
bool					s_Output_FilterPath	= false;			// use filter_path to return only took,errors on _bulk upload 
u32						s_Output_MergeMin	= 1;				// minimum number of blocks to merge on output 
u32						s_Output_MergeMax	= 64;				// maximum number of blocks to merge on output 
						...
						s32* CPUMap)
{
	...
	fprintf(stderr, "   BufferCnt     : %i\n", Output_BufferCnt); 
	fprintf(stderr, "   KeepAlive     : %i (%.3f sec)\n", Output_KeepAlive, Output_KeepAliveTimeout / 1e9); 
	...
	s_Output_ESTimeout 	= Output_KeepAliveTimeout;

	s_Output_FilterPath = Output_FilterPath;
	...
	fprintf(stderr, "   Output MemoryAlloc   : %i MB\n", (u32)(MemoryAlloc / kMB(1)) );	
...
void Output_ESHostAdd(Output_t* Out, u8* HostName, u32 HostPort)
{
	...
	fprintf(stderr, "   Output ESHost %s:%i\n", HostName, HostPort);
...
static int ConnectToES(u8* IPAddress, u32 Port)
{
	...
	struct timeval tv;
	tv.tv_sec  =  s_Output_ESTimeout / (u32)1e9;
	tv.tv_usec = (s_Output_ESTimeout % (u32)1e9)/ (u32)1e3;
	...
	int timeout = s_Output_ESTimeout / 1e6;  // user timeout in milliseconds [ms]
...
//Bulk[RawLength/2] = '{';
	...
	*/
	...
	if (!s_IsESNULL)
	{
		...
		// HTTP request
		if (s_Output_FilterPath)HeaderPos += sprintf(Header + HeaderPos, "POST /_bulk?filter_path=took,errors HTTP/1.1\r\n");
...
u64 Output_BufferAdd(Output_t* Out, u8* Buffer, u32 BufferLen, u32 LineCnt)
{
	...
	if (Out->ESPush)
	{
		...
		while (true)
		{
			...
			bool IsStall 	= false;
			if ((Put - Out->BufferFin) > (Out->BufferMax - Out->CPUActiveCnt - s_Output_MergeMax))
			{
				IsStall = true;
			}
			else
...
static void* Output_Worker(void * user)
{
	...
	while (!s_Exit)
	{
		...
		else
		{
			...
			for (int b=0; b < s_Output_MergeMax; b++)	
			{
			...
			bool IsOutput = false;
			if (BufferCnt >= s_Output_MergeMin)
			{
				IsOutput = true;
		 		Out->BufferGet = Get;
			}
